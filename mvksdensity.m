...
[yData,n,d,ymin,ymax,xispecified,xi,u,npoints,kernelname,...
...
[ty,~,weight,u,foldpoint,maxp] = apply_censoring_get_bandwidth(cens,yData,ty,n,ymax,weight,u,d,kernelname);
...
yData(any(isnan(yData),2),:) = [];
if isempty(yData)
    error(message('stats:mvksdensity:NanX'));
end
[n,d] = size(yData);
...
okargs_2   = {{'npoints','NumPoints'} 'plotfcn'};
defaults_2 = {          30             'surf'};
okargs_1   = { {'npoints','NumPoints'} 'censoring'};
defaults_1 = {          1000             false(n,1)};
...
if d==1
    ...
    if d==2
        ...
        [u,kernelname,support,weight,ftype,cutoff,bdycorr,npoints,plottype,~,extra] = ...
            internal.stats.parseArgs(okargs, defaults, varargin{:});
        if ~xispecified
            npoints = floor(sqrt(npoints)); %npoints = 30;
        else
            npoints = [];
        end
...
% -----------------------------
function [ty,ymax,weight,u,foldpoint,maxp] = apply_censoring_get_bandwidth(cens,yData,ty,n,ymax,weight,u,d,kernelname)
