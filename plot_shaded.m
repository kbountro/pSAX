...
%
% John Onofrey (2020). Shaded Plots and Statistical Distribution Visualizations 
...
x_min = min(X);
x_max = max(X);
V_alpha = alpha_value*((X-x_min)/x_max);
F = [1 2 3 4];

hold on
% Create patches for the shaded region
for i=1:numel(Y)-1
    V = [ x_min Y(i) ; X(i) Y(i); X(i+1) Y(i+1); x_min Y(i+1) ];
